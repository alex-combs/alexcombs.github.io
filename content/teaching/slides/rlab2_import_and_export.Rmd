...
- The RStudio import window in the bottom-right provides the code needed to import the data based on the options we chose

- **Important:** You must paste this code in your Rmd or else it won't knit if you use code that requires the data

--

> **Copy the code. Hit Cancel. Paste code in the setup code chunk**
...
> **Delete loading of packages since we already loaded `tidyverse`, which includes the function to import CSV.**

> **Delete the file path and include only the file name.**
...
- If we opened with spreadsheet software, we would see that the data we want to import "ga_schdist_raw.xlsx" runs from cell A7 to cell I230
...
- May not *have* to rename variables; typing long variables can be inconvenient but R's auto complete feature saves time

- Recommend renaming variables if they contain a blank space (e.g. "free lunch")

--

```{r, eval=FALSE}
dataset <- rename(dataset, new_name=old_name, new_name=old_name,...)
```

---
# Renaming variables

Step 1: Get a list of current variables you can copy-paste to code chunk

```{r, eval=FALSE}
names(dataset)
```

> **Add `names(ga_schdist_raw)` to the _bottom of setup code chunk_ and run**

> **Copy-paste output to the same code chunk**

---
# Renaming variables

Step 2: Add the necessary `rename()` code around the list of current variable names

> **Add following code**

```{r, eval=FALSE}
ga_schdist_raw <- rename(ga_schdist_raw, list_of_current_variables)
```
...
```{r, eval=FALSE}
ga_schdist_raw <- rename(ga_schdist_raw,
...
- Will likely need to change some style and formatting

---
# Knit to PDF

- If you want to knit to PDF, you can install a light version of LaTeX within RStudio. 
...
- We have now covered the basic workflow of most data application projects
