...
# Let's pause learning data imports to learn some ways to print tables.
...
- Note both tables are readable but look pretty bad
...
- The `gap_preview` table looks good but the `hc` table runs off the page.
...
> **Add the code chunk option that will suppress the code AND the output related to `gap_preview`**

--

> **Add the code chunk option that will suppress the code but show the output related to `hc`.**

> **Knit to HTML again to check the export**
...
- Organizations publish data files in a variety of (inconvenient) formats.

- But a given organization tends to use a consistent format across files and time

--

- Some guidelines:
  - Don't overwrite the original file, use code to change how it is formatted
  - Improve the formatting at least enough to eliminate unnecessary rows/columns
  - Decide whether additional formatting is worth your time (renaming variables, changing variable types)

---
# Import CSV

> **Download "ncbirths.csv" from eLC and add it to your "rlab2" project folder**

- Some notable features of this file (viewing in Excel):
...
- The above code uses options to:
  ...
  - Store all variables as integers, numbers, or factors
  - Skip the first row of variable names (because inserting my own row of names)

---
# Import Excel

- To import Excel files, use the `read_excel` function in the `readxl` package.
...
- Notable features for this file (viewing in Excel):
  - Variable names start on 7th row
  - Variable names are messy
  - Several rows we should not import starting after row 230

- We need to use options for this file. Let's check out what options `read_excel` has via the "Help" tab in the bottom-right pane.

---
# Import Excel

- The `range` option allows us to specify the cells we want to import from top-left to bottom-right
...
---
# Point-click-and-code option

- For virtually any data file in your project folder:

- Click on the file in your files pane (bottom-left), then click "Import Dataset..."

- R will choose the package and function needed to import that file and provide a preview

- Allows you to point-and-click based on what options you want to use to improve the formatting

- Displays and updates the code to complete the import. You can then copy-and-paste this code to your Rmd.
