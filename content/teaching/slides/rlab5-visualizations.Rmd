...
- Create an appropriate graph according to your intended message and available data
...
- Let's save the most recent date with data and name it `today`
...
```{r}
covid_ga <- covid_ga %>% # take covid_ga
  group_by(county) %>% # and for each county
  arrange(date) %>% # sort date in ascending order
  mutate(day_cases = cases - lag(cases), 
         day_deaths = deaths - lag(deaths)) %>% # subtract yesterday from each day
  ungroup() # stop grouping by county
```
...
```{r, eval=FALSE}
summary(sum.covid_ga) # remember to add results='asis' inside curly brackets
```
...
- Remember to include only `today` observations
...
- Rather than total cases or deaths for all counties as of the most recent date, let's focus on the distribution of daily counts for a single county.
