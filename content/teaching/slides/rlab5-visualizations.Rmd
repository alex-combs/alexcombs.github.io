...
> **Start a new project following whatever process works best for you.**
  
> **Start a R Markdown document. Keep the setup code chunk at the top but delete the rest of the template. Change echo to FALSE. Add message=FALSE and warning=FALSE.**
...
- Let's examine the latest COVID data for every U.S. county provided by the New York Times
...
- These data include **cumulative** counts (not daily) of COVID cases and deaths from January 1, 2022 to yesterday

- Additional data can be found [here](https://github.com/nytimes/covid-19-data)
...
- Note the latest date with data is yesterday. We will want to focus on the most recent counts, so let's save end_date's date for easy reference.
...
- Now we can add `end_date` to our code any time we want to include the most recent data. We only need to update `end_date` to update any graph we make.
...
- When numbers are large, R will sometimes display them in scientific notation

- For example, `2e+05` on previous graph is equivalent to 200000 (2 followed by 5 zeros)

- You can disable all scientific notation during the current R session by adding `options(scipen = 999)` as a separate line in your setup code chunk

- Another way is to specify how we want R to display graph labels for each axis
