...
library(data.table)
library(tidyverse)
library(lubridate) # you do not need to install this if tidyverse is already installed
```
...
- Note the latest date with data is end_date. We will want to focus on the most recent counts, so let's save end_date's date for easy reference.
...
- Now we can add `end_date` to our code any time we want to filter conditional on the most recent data, and we only need to update `end_date` to update any graph we make.
