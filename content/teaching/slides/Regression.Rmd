...
- Regression is a statistical method to
  - **Explain** the relationship between an outcome variable and one or more explanatory variables
...
- $y$ referred to as outcome, response, dependent variable
...
- The $\beta$s are **population parameters** we wish to estimate.
...
.pull-right[
- This equestion represents our sample regression equation
]
...
On average, a one-hour increase in study hours per week is associated with a 0.001 point increse in GPA.

Results indicate each additional hour students study per week inceases GPA by 0.001 point, on average.
...
.left-column[
- We can view the regression results for each observation
- We see the observed data, the predicted outcome, and the difference between observed and predicted
]
...
- Model 3 has the highest $R^2$, thus it is the best model
...
- Nothing fundamentally different; just more variables
...
- With the model for study hours, there is any number of additional factors contained in the error term $\epsilon$

- Generally, if we can, we should include those additional factors in the same regression model
  - Especially if one of those factors affect study hours
...
- All of these examples do a very poor job at explaining GPA, at least in for this sample of students