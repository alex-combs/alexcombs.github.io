...
load("lectures_files/evaluation.RData")

hisp <- evaluation %>% 
  filter(round==1) %>% 
  select(health_expenditures, treatment_locality, age_hh, educ_hh, dirtfloor, bathroom, hospital_distance)
...
- **Counterfactual:** The unobserved outcome from an alternate reality. 
...
- Use **quasi-experimental** methods that use a feature of the treatment that allows us to claim it is as good as randomized

--
  
- **Non-experimental:** Controlling for all the right variables to claim there is no spurious correlation
...
- Impact of health insurance subsidies on out-of-pocket health care expenditures.

```{r, echo=FALSE}
mod1 <- lm(health_expenditures ~ treatment_locality, data = hisp)

get_regression_table(mod1) %>% 
  kable(digits = 2)
```

- We are concerned whether this estimate is accurate. In other words, does the model eliminate bias?
...
- If you run a regression and the estimate is statistically significant (we haven't covered significance yet), you have established correlation that is unlikely due to random variation
...
- Easiest to claim for discrete events with a pre and post
...
- **External validity (generalizability):** can results be applied beyond the groups or context that were studied?
