...
> **TSet `acc_taxrev` as a TS object.**
...
```{r, fig.show='hide'}
autoplot(ts_house)
autoplot(ts_house[,1]) # specifies variable based on column
autoplot(ts_house[,2]) 
autoplot(ts_house[,3])
autoplot(ts_taxrev)
```
...
- GDP (column 3) exhibits some seasonality. Here are a few useful plots for seasonal patterns.
...
- Electricity production more than 2 years ago significantly correlate with values at a particular time
]
...
- These data suppose a 2% annual increase in housing prices and GDP over the next 12 quarters
...
- Thorough coverage of time series and forecasting is its own course
