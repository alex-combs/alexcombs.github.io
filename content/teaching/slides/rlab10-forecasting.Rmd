...
> **Download `acc_select_tax_rev`, `us_house_data`, and `start_forecast_data` from eLC and import. You can copy paste the below code in your setup code chunk.**

```{r, include=FALSE}
acc_taxrev <- read_csv("labs_files/acc_select_tax_rev.csv")
house_data <- read_csv("labs_files/us_house_data.csv")
starts_fc <- read_csv("labs_files/starts_forecast_data.csv")
```

```{r, eval=FALSE}
acc_taxrev <- read_csv("acc_select_tax_rev.csv")
house_data <- read_csv("us_house_data.csv")
starts_fc <- read_csv("starts_forecast_data.csv")
```
...
```{r, fig.show='hide'}
autoplot(ts_house)
autoplot(ts_house[,2]) # specifies variable based on column
autoplot(ts_taxrev)
```
...
- Regression-adjusted models incorporate explanatory variables
  ...
  - Example: predicted housing starts if median price increases 2% each year
...
- Past electricity production significantly correlates with production at a particular time
]
...
ggAcf(ts_house[,2]) # applied to first column, price
```
...
.pull-right[
- `ets` has fit a model with `M`ultiplicative error, `Ad`amped trend, and `A`dditive seasonality
]
---
# Exponential smoothing in R
...
- Can use `checkresiduals` to test validity of forecast
...
- Let's forecast housing starts incorporating price and GDP as explanatory variables

> **Add and run below time series linear model**

```{r}
reg_starts <- tslm(house_starts ~ median_homeprice + gdp + trend + season, 
                   data = ts_house)
```
...
- Now we can forecast starts in response to changes in explanatory variables using the `starts_fc` data we downloaded from eLC

- These data assume a 2% annual increase in housing prices and GDP over the next 12 quarters
