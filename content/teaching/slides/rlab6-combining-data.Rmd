...
- To help us visualize the goal of appending in R, I will demonstrate appending in Excel
...
- And like always, Excel doesn't allow the process to be easily replicated

- R can append data with much less effort
...
> **Immediately below your setup code chunk, insert a heading like Append**
...
- To help us visualize the goal of left joins in R, I will demonstrate it in Excel 
...
- Becomes labor intensive and prone to error if there were more data we needed to match to the primary dataset
...
- How is `county` stored in `covid_ga` and `ga_county_pop`? The `glimpse` printout tells us.
