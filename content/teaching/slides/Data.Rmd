...
# If the unit of analysis for our data is **year**, what variables might we collect?

---
class: inverse, middle, center

# If the unit of analysis is **people**, what variables might we collect?
...
# Which of our variables for our investigation of homelessness are qualitative or quantitative?
...
- Possible range of a values does not differentiate between discrete and continuous

  - Continuous variable can range between 0-1
  - Discrete variable can range from 0 to infinity
...
- Characteristics can be measured `between` or `within` units and/or `between` or `within` time periods

--

- Cross-sectional measures `between` units and `within` a time period

--

- Pooled cross-sectional measures `between` units and `within` a time period (sort of)

--

- Time series measures `within` a unit and `between` time periods

--

- Panel measures `between` units and `between` time periods
