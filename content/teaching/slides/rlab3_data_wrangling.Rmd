...
- Learning analysis is not very useful if you can't realistically prepare data
...
> **Start a new project named "rlab3". I recommend locating it in the same parent file for your rlab 1 & 2 folders.**
...
- Virtually all wrangling can/should be done with the tidyverse package developed by Hadley Wickham
...
> **Inside your setup code chunk, load both datasets into R.**

> **Click on the datasets in your environment pane to view the data.**
...
- Variable names should be as short as possible and most consider snake_case_format to be best
...
> **Start a new line inside the code chunk with `rename` and use `glimpse` on the `st_fin` dataset. Add the option that suppresses code and output.**
...
- The `ga_schdist_raw` has many variables in need of a new name.
...
> **Download the `ga_schdist_raw.xlsx` file from eLC and add to your project folder.**
...
- From glimpsing the data, we see that `st_fin$state` is stored as a character variable

- Suppose we want R to recognize it as a categorical variable

> **In the code chunk where we created `st_fin`, overwrite `st_fin$state` from text to categorical.**
...
- Let's create a copy of `ga_schdist_raw` that we will manipulate moving forward.
