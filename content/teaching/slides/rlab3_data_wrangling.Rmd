...
- print a few of the top or bottom rows using `head` and `tail`
- combine functions in one chunk using the pipe operator `%>%`
- compute summary stats using `summarize` and `group_by`
...
- I already wrangled these data via the import
...
```{r}
stf <- rename(state_finance_16, 'id' = 'id2', 
                           'id2' = 'id1')
```
...
- Start a code chunk and run the following code
...
- Could rename using the same process
- There is a quicker way

---
# Renaming many long variables

- Add the following to the same code chunk 
...
```{r}
schdist_newnames <- c("district", "state", 
                      "charter", "enroll", 
                      "lep", "sped", 
                      "frpl", "white", 
                      "expenditures")
...
```{r, message=FALSE}
ga_schdist <- rename_at(ga_schdist_raw, 
                        vars(schdist_oldnames), 
                        ~ schdist_newnames)
```

- You can apply this code to any dataset.

- Show reader the new `stf` and `ga_schdist` names using `glimpse`
...
- From glimpsing the data, we see that `stf$state` is stored as a character variable
...
- Several character variables in `ga_schdist` need converted to numeric
...
```{r, eval=FALSE}
filtered_data_set <- filter(unfiltered_data_set, criteria)
```
...
- Insert code chunk and run following code

```{r}
stf_filtered <- filter(stf, 
...
- Use `unique` to examine the different levels for `stf_filtered$finance_source`

- Use filter to keep only the rows with total revenue or total expenditure

- Name the new object `stf_filtered`, which will overwrite the existing object

--

```{r, eval=FALSE}
unique(stf_filtered$finance_source)
```

```{r}
stf_filtered <- filter(stf_filtered, 
...
- Sometimes it makes sense to remove or keep rows or columns by their position in the dataset
  - Position in terms of row or column number in the dataset

- Recall that the last row in `ga_schdist` shouldn't be included because it includes totals, which is analysis, not data

- Suppose we want to remove that row by identifying the row number (subsetting)
...
```{r, eval=FALSE}
dataset <- dataset[row number(s) to keep or remove, 
...
dataset <- dataset[c(-3,-5), c(1:10)]
# would drop 3rd and 5th rows, and columns 1 to 10
```

---
# Subsetting

- Drop the last row of `ga_schdist` along with the `state` variable since it is somewhat unnecessary

--

```{r}
ga_schdist <- ga_schdist[-224, -2]
```
...
- The `ga_schdist` data has several absolute counts, such as `expenditures`
...
```{r}
ga_schdist <- mutate(ga_schdist, exp_per_pupil = expenditures/enroll)
```
...
- Four GA districts with lowest FRPL
...
- Four GA districts with highest FRPL
...
- Instead, we can get to the end result without creating an intermediate dataset
...
- where function is one of many possible summarizing functions (see dplyr cheatsheet)
