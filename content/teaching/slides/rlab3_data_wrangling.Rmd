...
```{r}
state_finance_16 <- rename(state_finance_16, 'id' = 'id2', 
                           'id2' = 'id1')
```
...
```{r, message=FALSE}
ga_schdist_raw <- rename_at(ga_schdist_raw, 
...
- Recall that the last row in `ga_schdist_raw` shouldn't be included because it includes totals, which is analysis, not data
...
- Drop the last row of `ga_schdist_raw` along with the `state` variable since it is somewhat unnecessary

--

```{r}
ga_schdist_raw <- ga_schdist_raw[-224, -2]
```
