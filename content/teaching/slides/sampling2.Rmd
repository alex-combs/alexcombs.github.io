...
1. We take a sample. Hopefully **random**. If not, need to correct using sample weights or controls in a regression. Otherwise, **biased**.
...
- We *know* a (fair) coin has 50/50 chance landing heads/tails

- Suppose we didn't know this, as is the case with more important phenomena, and wanted to estimate the probability of heads

- LLN says that, given a large enough sample size, our estimate will approach the population parameter

---
# Law of Large Numbers (LLN)

> **Run the two `coin-flip` code chunks**

--

> **Run the `head-count` code chunk with its default of 10 flips**

--

> **Change the number of flips to 100 and run again**

---
class: inverse, center, middle

# As sample size increases, accuracy increases (assuming no bias)
...
- What if we collected all of the estimates we got from each of our samples and plotted their distribution? That would give us a **sampling distribution** for our estimate.
...
> **Let's compute the standard error of our sampling distribution**
...
> **Complete the `confidence-interval` code chunk then run**
...
> **Run the `population` and `parameter` code chunks.**

---
# Sample and estimate

> **Run the `sample` and `salary-estimate` code chunks.**

--

- This time, we won't compile our estimates to make a sampling distribution.

- Instead, we assume our estimate is a single draw from a sampling distribution that is approximately normal

- But we still need the standard error of the sampling distribution...
